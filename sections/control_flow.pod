...
There is no C<else if> construct; this is a syntax error:
...
variable (C<$_>) to each in turn.  The topic variable is also called the default
variable (L<default scalar variable>).  Perl executes the block for each integer
...
[Should you provide the output from some, many, most, all of your examples?]
...
exit an iteration with C<next>.  You may use it with may be used with a
C<while>, C<until>, C<with>, or C<for> loop.  Examples of C<continue> are rare,
but it's useful any time you want to guarantee that something occurs for every
