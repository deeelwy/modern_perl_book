...
[This needs more explanation, or should just be deleted.]
You may assign a numeric value to C<keys> to modify the expected size of the
hash.  This is a minor optimization you almost never need.
[When would I actually need this optimization?  Do people really use it.  I
almost wanna write a PPI script to spider all of CPAN to see if this is ever
used.  This side bar asks an interesting question, but doesn't answer it.
Please answer it, or don't ask the question.]
...
The C<while> loop is important to the C<each> example, as you cannot declare
multiple lexical loop iterator variables in a C<for> loop.
...
Note that, as with array slices, the sigil of the hash changes to indicate list
context.  You can still tell that C<%addresses> is a hash by the use of the
curly braces to indicate keyed access.
