...
=head4 Special File Handling Variables
...
filehandle.  Be sure to load C<IO::File> first, as you cannot call methods on
...
X<IO::File; input_line_number()>
X<IO::File; input_record_separator()>

Once you have loaded C<IO::File>, you may also use its C<input_line_number()>
and C<input_record_separator()> methods instead of C<$.> and C<$/>
respectively.  See C<perldoc IO::File> and C<perldoc IO::Handle> for more
information.
...
value.  C<perldoc -f -X> lists several other file tests; the most popular are:
