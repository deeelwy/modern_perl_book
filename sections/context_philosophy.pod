...
assignment forces.  Use the C<scalar> operator to enforce it:
...
code fails because the value of both strings when treated as numbers is C<0> due
to L<numeric_coercion>).
