...
[Does "escaping" need to be explained?]
...
backslash, lest the Perl parser believe you are trying to escape the trailing
single quote:
...
Any other backslash appears literally in the string, but you can escape
backslashes safely:
...
represent text in most languages which use the Latin script.
...
See C<perldoc perluniintro> for far more detail Unicode, encodings, and how to
manage incoming and outgoing data in a Unicode world.
...
is always occurs in list context.  Indexing into an array can occur in scalar
